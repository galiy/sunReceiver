...
//go:embed mapsettings/catalog.json
...
func readCellRanges(ctx context.Context, c *modbusmap.Client, addrs []uint16) (map[uint16]byte, []string) {
...
func readMapSettingsSnapshot(ctx context.Context, target mapSettingsTarget) (*mapSettingsSnapshot, error) {
	...
	c := &modbusmap.Client{Address: target.address(), Unit: target.unit}
	defer c.Close()
...
func applyMapSettings(ctx context.Context, target mapSettingsTarget, changes map[string]float64) (map[string]string, error) {
	...
	if err := c.WriteCommand(ctx, modbusmap.ComMAPEEPromWR); err != nil {
	...
	if err := c.WriteCommand(ctx, modbusmap.ComMAPCallLoadEEProm); err != nil {
...
var mapSettingsActions = []mapAction{
	{Key: "on", Name: "Включить МАП", Desc: "Включить генерацию (ComMAP_ON)", Cmd: modbusmap.ComMAPON, Confirm: true},
	{Key: "off", Name: "Выключить МАП", Desc: "Выключить генерацию (ComMAP_OFF)", Cmd: modbusmap.ComMAPOFF, Confirm: true},
	{Key: "charge_on", Name: "Включить заряд", Desc: "Включить заряд от сети (ComMAP_ChargeON)", Cmd: modbusmap.ComMAPChargeON, Confirm: true},
	{Key: "charge_off", Name: "Выключить заряд", Desc: "Выключить заряд от сети (ComMAP_ChargeOFF)", Cmd: modbusmap.ComMAPChargeOFF, Confirm: true},
	{Key: "stat_reset", Name: "Сброс статистики", Desc: "Сбросить накопленную статистику (ComMAP_StatReset)", Cmd: modbusmap.ComMAPStatReset, Confirm: true},
	{Key: "disch_off", Name: "Запретить разряд", Desc: "Выключение генерации по полному разряду (ComMAP_DischOff)", Cmd: modbusmap.ComMAPDischOff, Confirm: true},
	{Key: "load_eeprom", Name: "Загрузить настройки из EEPROM", Desc: "Инициализация данных из EEPROM (ComMAP_Call_load_EEProm)", Cmd: modbusmap.ComMAPCallLoadEEProm, Confirm: false},
	{Key: "reset", Name: "Сброс контроллера", Desc: "Полная перезагрузка МАП — крайняя мера (ComMAP_Reset)", Cmd: modbusmap.ComMAPReset, Confirm: true},
}
