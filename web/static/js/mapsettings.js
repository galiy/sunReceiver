// sunReceiver — страница «Настройки МАП» (map-settings).
...
(function () {
  ...
  var DEF_MODE = body.getAttribute('data-map-mode') || 'dominator';
  var DEF_IP = body.getAttribute('data-map-ip') || '';
  var DEF_PORT = body.getAttribute('data-map-port') || '502';
  var LS_MODE = 'srMapSettings.mode';
  var LS_IP = 'srMapSettings.ip';
  var LS_PORT = 'srMapSettings.port';
  ...
  function readAll() {
    ...
    api('/api/map-settings' + qs).then(function (snap) {
      renderSnap(snap);
    }).catch(function (e) {
  ...
  function readTime() {
    ...
    var t = target();
    var qs = '?mode=' + encodeURIComponent(t.mode) + '&ip=' + encodeURIComponent(t.ip) + '&port=' + t.port;
    api('/api/map-settings/time' + qs).then(function (st) {
