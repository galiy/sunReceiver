...
Go-приложение, которое опрашивает solar-инверторы через их WiFi-даталоггеры (Solarman LSW-3/LSE, порт 8899, TCP) и сохраняет распарсенные данные в Redis (с persistence RDB+AOF). При полностью пустом Redis данные восстанавливаются из PostgreSQL. Включает веб-дашборд текущих параметров.
...
Список опрашиваемых инверторов задаётся в **`sunReceiver.json` рядом с исполняемым файлом** (`os.Executable()`; при `go run .` — fallback в CWD). Структура: раздел **`invertors`** — список инверторов `[{"ip", "name", "type": "deye"|"sofar", "logger_sn": uint32, "disabled": bool}]`; `name` — логическое имя (обязательно); **`disabled` — ОБЯЗАТЕЛЬНОЕ поле** (`false` = опрашивается, `true` = временно отключён — устройство в конфиге, но не опрашивается, при старте логируется; отсутствие поля = ошибка загрузки конфига). МАП Титанатор — **отдельный раздел `map`** `{"name", "ip", "unit", "disabled"}` (не входит в `invertors`); **`disabled` — ОБЯЗАТЕЛЬНОЕ поле** (`false` = МАП опрашивается через Modbus TCP, unit по умолч. 1; `true` = пулер по Modbus НЕ запускается, а параметры батареи/сети берутся из веб-API ПАК «Малина» `read_json.php?device=map` — для этого обязателен настроенный раздел `mppt`). MPPT-контроллеры — раздел **`mppt`** `{"base_url", "mppt_path", "login", "password", "bms_path"}` (пароль в открытом виде, файл в git не выгружается): `bms_path` (необязательное, напр. `/read_bms.php`) включает опрос ANT BMS — тот же хост/Basic-auth, что и MPPT (см. секцию «ANT BMS» ниже). Расположение баз — раздел **`db`** `{"redis": "host:port", "pg": "DSN"}` (источник дефолтов флагов `-redis`/`-pg`; пароль PG — в DSN, конфиг приватный). Счётчик DDS238 — раздел **`meter`** `{"name", "ip", "port", "unit", "first_reg", "register_count"}` (имеет приоритет над legacy-файлом `dds238.json`, см. секцию «Электросчётчик DDS238»). Опрос Deye/Sofar — раз в 10 секунд; МАП и MPPT — 1 раз в секунду (с сохранением 1 точки за 10 с); BMS — 1 раз в секунду (отдельный Redis-ключ + ряд 5-минутных усреднённых точек, см. «ANT BMS»).

**Шаблон `sunReceiver.sample.json`** (в git) — публичный пример структуры конфига. **Всегда** обновлять его при любом изменении структуры/содержимого `sunReceiver.json` (новые разделы, поля, типы устройств): IP-адреса заменить на случайные из `192.168.0.x`, серийные номера — на случайные, пароли — замазать (`CHANGE_ME` / `CHANGE_ME_PASSWORD`). Сам `sunReceiver.json` приватный (в `.gitignore`), sample — публичный.
...
- `solarman/` — пакет-клиент Solarman V5: `BuildReadFrame` (12-байтный datafield, для Sofar), `BuildDeyeReadFrame` (15-байтный datafield + реальный SN, для Deye; и вариант с произвольной Modbus-функцией `BuildDeyeReadFrameFn`/`ReadRegistersDeyeFn` — для чтения func 04 HW-диапазона Sofar), `SplitFrames` (длина кадра = **11** + PayloadLen + 2, префикс A5+len+control+serial+SN), `ParseModbusPDU` (01 03/04 | bytecount | data | crc16 LE), `Checksum8` (sum[1:len-2] mod 256), `CRC16Modbus` (стандартный, в PDU пишется LE).
- `main.go` — poller: каждый инвертор (Deye/Sofar) — в своём независимом цикле `runInverterPoll`, раз в 10 с (завис/таймаутит один — остальные продолжают без влияния, снимок пишется в Redis сразу по готовности с фактическим временем). Список целей читается из **`sunReceiver.json` рядом с бинарником** (`os.Executable()`; при `go run .` — fallback в CWD) через `loadConfig` в `targets []invTarget` {IP, Name, LoggerSN, Kind}: раздел `invertors` → `type` "deye"→`kindDeyeString`, "sofar"→`kindSofar` (поле `disabled` обязательно; `true` пропускается); раздел `map` → `kindMAP` (MPPT — `kindMPPT` — в конфиг НЕ задаётся, появляется динамически см. секцию МАП). Источник данных МАП задаёт обязательное поле `disabled` раздела `map` (`false` = Modbus TCP, `true` = веб-API ПАК «Малина» `read_json.php?device=map`, нужен раздел `mppt`). Название «**логическое имя**» инвертора (обязательное поле, напр. `Deye Left`, `Sofar-2.5`, `Bineos Right`; для MPPT имя генерируется как `MPPT-<slot+1>`). Deye — реальный SN даталоггера (`ReadRegistersDeye`), Sofar — тоже `ReadRegistersDeye` с 15-байтным datafield и реальным SN (проверено 2026-09-07: 12-байтный `ReadRegisters`/`BuildReadFrame` Sofar LSW-3 отвечает только heartbeat с кодом 0x05; на 15-байтный кадр — полный блок 0x0000-0x0027). Маппинг регистров: Sofar — в `mapSofarRegisters` (регистрации 0x0000-0x0027), Deye string — в `deyeRegMap`/`mapDeyeRegisters` (у каждого сенсора `Tag` — имя контракта в values). **Серийные номера в снимке**: `device_sn` (SN даталоггера, **десятичный**, из поля кадра `DeviceSN`) и `inverter_sn` (SN самого инвертора). Для Deye `inverter_sn` — ASCII-строка в регистрах **0x0003–0x0007** (10 цифр, проверено: .70="2405018274", .79="2405018294", .91="2103014266", .92="2103014293", .93="2103014015"); для Sofar — ASCII-строка HW-диапазона func **04** **0x2000–0x200D** (регистр 0x2000 = длина, далее ASCII 2 байта/регистр; может быть НЕ числом — строка, напр. .76="SA3ES127LC1055", .95="SA3ES033KAB575"). Суффикс версий платформ (`V<цифры>V<цифры>V<цифры>`, напр. `V480V100V480`) отрезается `trimSofarVersions` (на дашборде как серийный номер не выводится). Сборка строки — `asciiFromRegisters`. Для чтения func 04 добавлены `BuildDeyeReadFrameFn`/`ReadRegistersDeyeFn` (в `solarman/`). МАП (kindMAP: Modbus TCP или веб-API ПАК «Малина» по `disabled`) и MPPT (kindMPPT, веб-API ПАК «Малина») — в отдельном 1-сек цикле `runMapPoll` (см. секцию МАП). **Poller пишет СРАЗУ только в Redis** (`redis_store.go`) — как live-хранилище; в PostgreSQL (`pg_store.go`) данные НЕ пишутся напрямую — туда идёт усреднённые 5-минутные точки от фонового процесса аккумуляции (см. `accumulator.go`). Флаги CLI: `-redis <addr>`, `-pg <dsn>` (дефолты берутся из раздела `db` конфига; если раздела нет — `127.0.0.1:6379` / `postgres://localhost:5432/sunreceiver?sslmode=disable`; пустая строка у `-pg` выключает PG), `-pg-restore-window <dur>` (по умолч. `720h`; `restoreRedisFromPG` всё равно обрезает окно до `recentCutoff` — 2 календарных суток, т.е. значения >48h не действуют), `-dashboard <addr>` (по умолч. `:8080`, пустая строка выключает). **Чистое завершение**: по сигналу main закрывает `stopBG`, ждёт завершения ВСЕХ фоновых горутин (`bgWg.Wait()` — инверторы, аккумулятор, чистка, МАП/MPPT, BMS, счётчик, реставрация, дашборд — он останавливается через `http.Server.Shutdown`, бюджет 5 с) и только потом defer'ы закрывают пулы Redis/PG: записи при остановке (BMS-drain, in-flight `averageBucket`, последний снимок) не гоняются с `pool close`. Веб-дашборд — `dashboard.go`. Снимок (структура `deviceSnapshot`: **`{name, ip, timestamp, device_sn, inverter_sn, values}`**) отправляется только при успешном чтении данных (`HasData`); при `heartbeat_only`/`no data`/ошибке не сохраняется. `name`/`ip` — изsunReceiver.json (для kindMAP-слота/kindMPPT `ip` — `devKey`, напр. `IP#mppt0`); `values` — **универсальный контракт** (см. ниже). **`raw_registers` в снимок НЕ пишется** — только общие теги `commonContractTags`.
...
- **Поток данных**: логгер → poller → Redis (сразу, каждые 10 с). Отдельный фоновый процесс `runAccumulator` в `accumulator.go` каждый час по завершении очередного 5-минутного промежутка НЕ сразу усредняет его, а ждёт отсрочку `avgDelay` (2 мин, `accumulator.go`), чтобы собрать запаздывающие снимки медленных инверторов, затем читает из Redis накопленные за промежуток снимки, усредняет их (`averageValues`) и пишет одну точку в PG (`pgStore.InsertAveraged`). Прямой записи в PG из poller нет.
...
- Redis запускается **с persistent storage**: `redis-server --port 6379 --dir <data-dir> --save 900 1 --save 300 10 --appendonly yes` (RDB-снимки + AOF), данные живут в `<data-dir>` (используется `~/.cache/sunreceiver-redis`), поэтому не исчезают при перезапусках — это локальный инстанс для разработки; в проде Redis и PG — на сервере 253 (адреса — из раздела `db` конфига). Клиент — `github.com/redis/go-redis/v9` (ReadTimeout 3 с / WriteTimeout 2 с — зависший Redis не замораживает пулеры). При **полностью пустом** Redis при старте (например, очистили Redis) poller восстанавливает в нём данные из PostgreSQL: ряды инверторов/МАП/счётчика (`restoreRedisFromPG` → `pg.Averages` за окно 2 календарных суток) **и ряд 5-минутных усреднённых точек BMS** (`pg.BMSAveragesAll` → `sunreceiver:bms:series:<YYYY-MM>`), после чего persist их сохраняет.
- Ключи:
...
- `pgStore.Averages(start, end, ips ...string)` — чтение усреднённых точек за период (для дашборда и реставрации Redis); необязательный `ips` ограничивает выборку устройствами.
...
- `runAccumulator(store, pg, stop)` — фоновый процесс усреднения: (1) `backfillAccumulator` конвертирует уже накопленные в Redis за 2 календарных суток снимки в PG (однократно при старте, чтобы не потерять промежутки до начала аккумуляции); (2) далее каждый час по завершении 5-минутного промежутка (граница `nextBoundary`/`floorToStep`, 12 раз в час) ждёт отсрочку `avgDelay` (2 мин, меньше `avgStep` 5 мин — цикл не дрейфует), затем `averageBucket` читает из Redis снимки промежутка, усредняет и пишет в PG. Граница `b` завершённого промежутка фиксируется при срабатывании таймера, а сам `averageBucket` выполняется в отдельной горутине (медленное усреднение не влияет на планировщик границ); при stop дожидается завершения запущенных `averageBucket` (см. «Чистое завершение» в описании main.go).
...
- HTTP-сервер отдаёт HTML и JSON API. `/api/current` — `HGETALL sunreceiver:current` (последнее состояние). `/api/series` — временные ряды `ac_active_power` по инверторам, а также ряды МАП `map_grid_voltage`/`map_grid_power`/`map_battery_voltage`/`map_battery_power`. Все series-API (`/api/series`, `/api/tariffs`, `/api/bms/<name>/series`) принимают `from`/`to` в **RFC3339 (дата+время)**; страницы с графиками (/charts, /energy, /bms) шлют полные timestamps (`toISOString()`), поля «С/по» — `datetime-local` (дата+время; у /api/tariffs время внутри суток на выборку дней не влияет — важен календарный день в локальной зоне).
...
- **Усреднение длинных серий `/api/series`** (2026-09-16): после сборки каждой серии (инверторы, `total`, ряды МАП, ряды счётчика) проверяется её размер; если точек больше **1000** (`maxSeriesPoints`), диапазон `[from, to]` делится на **n = min(1000, кол-во секунд диапазона)** равных периодов (диапазон короче 1000 с — по периоду на секунду), и все точки одного периода схлопываются в одну: среднее значение (округление до 1 знака), метка времени первой точки периода, пустые периоды пропускаются. Реализация — `downsampleSeries` в `dashboard.go`; применяется только в `apiSeries` (страница «Графики»), `/api/tariffs` и BMS-серии не затрагиваны.
...
- **BMS (ANT батарея)**: группа с кнопками-батарейками (SOC, обновление 1 мин, `/api/bms`) + страница деталей `/bms/<name>` (1 с, `/api/bms/<name>`); детали — см. секцию «ANT BMS» выше.
- При отключённом PG (`-pg ""`) дашборд отдаёт только данные из Redis в пределах окна удержания (старше 2 суток — пусто).
- **Страница электроэнергии `/energy`** (кнопка «Электроэнергия» на главной) — посуточные и помесячные тарифы DDS238 из `daily_tariffs` (после переноса со страницы графиков): график «по дням» (по умолч. текущий месяц) + график «по месяцам» (по умолч. текущий год). У каждого графика **независимый** диапазон (пресеты + свой «С/по»), между собой не синхронизируются. Данные — `/api/tariffs?from=&to=` (RFC3339 дата+время; дни — по локальному календарю, по умолч. текущий месяц), `pgStore.DailyTariffsRange` (финализированные дни в диапазоне). Графики — бары на **time-шкале** (адаптер `chartjs-adapter-date-fns@3.0.0` в head страницы; точки: по дням — полдень дня, по месяцам — 1-е число месяца), поэтому **зум/сдвиг работают ТАК ЖЕ как на /charts** — за пределы загруженных данных: Ctrl+колесо — zoom-in/zoom-out, drag-панорама — сдвиг, кнопка «Сброс зума» (десктоп); щипок/свайп/двойной тап (mobile, `srTouchChart`). `limits.x.minRange`: 3 дня (по дням) / 90 дней (по месяцам). **После зума/сдвига данные перезагружаются под новое окно** (как на /charts, см. «Зум и панорама»): по завершении жеста 300ms-debounce → `energyWindowChanged` → `/api/tariffs` запрашивается заново под новое X-окно, поля «С/по» обновляются. Каждый график делает **СВОЙ независимый запрос** под своё окно (`ENERGY_PANELS`, per-panel debounce) — два графика масштабируются **независимо** (без синхронизации). Плагин chartjs-plugin-zoom загружается на странице. **Важно**: drag-панорама плагином (Hammer-рекогнизитор Pan) требует загрузки **hammer.js** — скрипт `hammerjs@2.0.8` грузится ПЕРЕД chartjs-plugin-zoom на всех трёх страницах с графиками (/charts, /energy, /bms); без него `window.Hammer` отсутствует и плагин молча не инициализирует жесты (работает только Ctrl+колесо — нативный wheel-слушатель).
- **Зум и панорама (все страницы с графиками)** — только горизонтальные: `mode:'x'` в опциях chartjs-plugin-zoom у всех графиков (/charts, /energy, /bms/<name>), mobile pinch/pan через `srTouchChart` — тоже только по X. Старый вертикальный зум графика напряжений на /charts (`zoomMode:'xy'`, `limits.y/y1`) удалён 2026-09-16 — теперь все графики зумятся только по горизонтали, как и остальные. **Перезагрузка данных после зума/сдвига** (2026-09-16): старая «догрузка краёв» (`maybeExtendAndLoad`/`bmsMaybeExtend`/`maybeLoadMore`, отслеживание `edge:'left'/'right'`) удалена как ненадёжная; теперь по **завершении** жеста зума/панорамы (десктоп — колбэки `zoom.zoom.onZoomComplete` + `zoom.pan.onPanComplete` в опциях chartjs-plugin-zoom, у wheel внутри плагина ещё свой 250ms-debounce; touch — колбэк `onGestureComplete` в `srTouchChart` при завершении pinch/pan/двойного тапа) ставится 300ms-debounce, после чего данные **удаляются и грузятся с бэкенда заново под новое X-окно**, а поля формы «С/по»(+день) обновляются. На /charts и /bms окно X **общее для всех графиков страницы** (`srWindowChanged`/`bmsWindowChanged` → `selRange`=новое окно, `periodMode='custom'`, `preserveZoom=true`, `loadAll()`/`loadBmsCharts()`). **На /energy то же самое, но каждый график делает СВОЙ запрос под своё окно** (`energyWindowChanged`, per-chart debounce, `ENERGY_PANELS`): два графика (по дням / по месяцам) перезагружаются **независимо**, между собой не синхронизируются. На всех трёх страницах страж «окно == текущий период панели» пропускает повторную перезагрузку (в т.ч. от `resetZoom()` — он тоже шлёт `onZoomComplete`). Побочный эффект (все страницы с графиками): после зума+перезагрузки новое окно становится **базовым** (`opts.scales.x.min/max` вшиты через `preserveZoom`), т.е. «Сброс зума»/двойной тап возвращает именно к нему, а не к исходному полному периоду — вернуться к широкому периоду можно пресет-кнопками или полями «С/по». Окно зума/панорамы сохраняется при ПЕРЕСТРОЙКЕ графиков (периодическая раз в минуту на /charts, /bms; перезагрузка под новое окно на всех трёх страницах): `preserveZoom` + min/max вшиваются в opts ДО `new Chart()` (`renderChart`/`bmsRender`/`renderEnergyChart`) — иначе первый draw на полном диапазоне через afterDraw-синхронизацию (checkZoomSync на /charts, `bmsZoomSyncPlugin` на /bms) транзиторно «раскачивает» остальные графики страницы, и после обновления график «прыгает обратно» на полный период.
...
- **Offline-устройства на главной (ночь/авария)**: poller пишет в `current` только при успешном чтении, поэтому последний снимок оффлайн-инвертора живёт в Redis вечно. Сигнал «оффлайн» — возраст снимка: `STALE_MS = 20 минут` (JS `dashboard.go`, то же окно, что `TOOLTIP_MAX_GAP` на графиках; Go-сторона `/api/current` — свой `staleCutoff` 20 мин). Следствия: (1) в сводной таблице у stale-устройства «Актуально» — красное (`td.stale-time`), время недоступности — **второй строкой через `<br>`** (не растягивает колонки на mobile), **данные ниже серийных номеров НЕ выводятся** (пустые ячейки — значения устарели; исключение — «Выработка сегодня/всего»: последние зарегистрированные значения остаются, но серым `td.stale-keep`; серийные номера постоянны, остаются); (2) плашка «N инверторов онлайн» (с правильным склонением) считает только fresh; (3) `total_power`/`total_pv` в `/api/current` суммирует только fresh-устройства — плашка суммарной мощности ночью показывает «—», а не сумку устаревших значений заката. Строки мощностей («Активная»/«Реактивная») — сразу после серийных номеров, значения — ярко-светло-зелёные (`tr.p-power`, `#66ff99`). **Серийные номера не «пропадают» у оффлайн-инверторов**: на выключенном инверторе регистры SN читаются нулями (`asciiFromRegisters` → ""), поэтому `runInverterPoll` до сохранения снимка **донасывает отсутствующие `inverter_sn`/`device_sn` из предыдущего снимка** (`redisStore.CurrentOne`, аппаратный номер постоянен); 2026-09-15 был сделан разовый backfill 4 устройств в `current` (скрипт `/tmp/backfill_sn.py` на 253, одноразовый).
- **Мобильная версия (тачскрин, ширина ≤900px)** — те же страницы, иная раскладка + touch-жесты; общая часть — `mobileCommon` в `dashboard.go` (template-defines `mcss`/`mjs`/`mnav`, подключаются к каждой странице, активный пункт нижней навигации передаётся в `Execute` через `.active`; **`<script>{{template "mjs"}}</script>` включается ДО inline-скрипта страницы** — скрипты страниц синхронно вызывают `srTouchChart` при загрузке, при обратном порядке touch-жесты не привязываются (ReferenceError), регрессионный тест — `dashboard_mobile_script_order_test.go`): раскладка — стекированные группы, плашки 2×N, нижняя фиксированная навигация (Главная/Графики/Электроэнергия), сводная таблица — горизонтальный скролл с закреплённой первой колонкой, увеличенные touch-цели. Жесты (только coarse-устройства, `matchMedia('(pointer: coarse)')`): чипы легенды — **tap = вкл/выкл, long tap (≥450 мс) = только эта линия** (`srBindChip`, на десктопе остаётся клик/двойной клик); графики — **щипок двумя пальцами = зум, горизонтальный свайп = панорама, двойной тап = сброс зума** (`srTouchChart`, canvas `touch-action: pan-y` — вертикальный скролл страницы не блокируется; после завершения pinch/pan/двойного тапа вызывается опциональный `onGestureComplete` — на всех страницах с графиками (/charts, /bms, /energy) он триггерит перезагрузку данных под новое окно; на /energy — каждый график под СВОЁ окно, независимо; **хинт со значениями на touch отключён на всех страницах**: «Графики» — `onHover`/`drawCursorTooltip` по `SR_COARSE||__srTouched` (перерисовка хинта на каждый touchmove мешала зуму), BMS и «Электроэнергия» — встроенный tooltip Chart.js `enabled:false` по тому же условию); на touch chartjs-plugin-zoom отключён (`pan/wheel/pinch: enabled:false`, чтобы его Hammer не перехватывал жесты). Вьюпорт: `maximum-scale=1, user-scalable=no`.
...
## МАП Титанатор: MPPT («КЭС») — via HTTP read_json, батарея/сеть — via Modbus TCP или HTTP
...
- MPPT-контроллеры: источник данных — **веб-API ПАК «Малина»** `read_json.php?device=mppt` (HTTP Basic-auth; конфиг `base_url`/путь/логин/пароль — в разделе **`mppt`** sunReceiver.json, пароль в открытом виде, файл в git не выгружается). SSH-доступ к хосту ПАК для разработки — в `.kilo/malina-ssh.json` (в git не попадает, программой не используется). В `sunReceiver.json` MPPT **не регистрируются**: состав контроллеров определяется динамически по фактически подключённым к МАП/ПАК контроллерам из ответа API (каждый элемент массива = контроллер со своим `slot`). Контроллер появляется на дашборде, когда появляется в ответе, и исчезает, когда перестаёт отвечать (его ключ удаляется из `current` через `PruneMPPT`; чистка — только при УСПЕШНОМ ответе API: при ошибке запроса предыдущий состав сохраняется, одиночный сбой Малины не гонит все MPPT с дашборда). Имя генерируется как `MPPT-<slot+1>`. Напрямую видны параметры панелей, которых нет в Modbus-гейте: Vc_PV, Ic_PV, P_PV, V_Bat, I_Ch, P_Out + `timestamp` актуальности данных. Реализация — `mppt_api.go` (`mpptSite`, `FetchMPPTs`, `mapMPPTAPI`) + динамический сбор в `pollAndSaveMap`, коннектор к `read_json.php` описан в `docs/read_json.md`.
- Раздел **`map`** в конфиге → `kindMAP` — оставлен для **данных батареи и сети МАП на дашборд** (grid/battery). Цель `MAP (батарея/сеть)` (192.168.13.74, unit 1). Пер-слотовый MPPT через Modbus больше не опрашивается — MPPT переехали на HTTP. Источник данных kindMAP задаёт обязательное поле `disabled` раздела `map` sunReceiver.json (отсутствие = ошибка конфига):
  - `false` — **Modbus TCP** (`modbusmap/` + `mapClientFor`), как раньше;
  - `true` — Modbus-пулер НЕ запускается, параметры берутся из **веб-API ПАК «Малина»** `read_json.php?device=map` (`mpptSite.FetchMAP` + `mapMAPAPI` в `mppt_api.go`); обязателен полный раздел `mppt` (иначе `log.Fatal` при старте). Ключ устройства (devKey) и имя — те же (IP МАП из конфига), что и при Modbus, поэтому история на дашборде преемственна при переключении режимов.
...
- MAP Modbus (`kindMAP`, `mapMAPRegisters`): `l1_voltage`=АКБ, `l1_current`=ток заряда (**в режиме заряда MODE 0x400==4 — со знаком «минус**, согласовано с API-веткой `mapMAPAPI`, где `_Iacc` знаковый), `ac_active_power`=V×I, `grid_frequency`=0, и **добавлено для дашборда**: `grid_voltage` (`_UNET` 0x422, 0 → нет сети, иначе +100 В), `grid_power` (`_PNET` 0x59A/0x59B, sign `_PNET_Sign_P` 0x587), `battery_voltage` (= `_UAcc_med` 0x405/0x406, `(VH*256+VL)/10`), `battery_power` (`_PLoad` 0x59E/0x59F, `((H*256+L)/8)*100`).
- MAP API (`kindMAP`, `mapMAPAPI` — при `map.disabled=true`): тот же контракт из `read_json.php?device=map`: `battery_voltage`/`l1_voltage` = `_Uacc`, `l1_current` = `_Iacc`, `ac_active_power` = `_Uacc×_Iacc`, `grid_frequency` = `_TFNET`, `grid_voltage` = `_UNET` (уже в В, без смещения +100), `grid_power` = `_PNET_calc` (= `_UNET`×`_INET`, достоверная; сырой `_PNET` у МАП занижен ~в 5 раз против счётчика, поэтому не используется — фолбэк на него только при отсутствии `_PNET_calc`), `battery_power` = `−_PLoad` (инверсия знака — в API `_PLoad` для отдачи отрицательный, контракт Modbus требует отдачу положительной). Время снимка — текущее время опроса, а не поле `timestamp` ответа API (МАП отдаёт его устаревшим/некорректным, что увело бы точки на годы в прошлое).
...
Конфигурация — раздел **`meter`** `sunReceiver.json` (`{"name","ip","port","unit","first_reg","register_count"}`;
подставные значения — в `sunReceiver.sample.json`); legacy-файл **`dds238.json`** рядом с бинарником
(fallback в CWD при `go run .`) поддерживается, в git НЕ коммитится (его шаблон `dds238.json.sample`
удалён из git); полное описание — `docs/dds238-meter.md`. Приоритет: раздел `meter` > файл `dds238.json`. Загрузка — `loadMeterConfig`
(`meter_config.go`). **Валидация**: если раздел `meter` в конфиге ЕСТЬ, но некорректен
(неполный `ip/port/unit/name`, либо `first_reg != 0` — декодер заточен под регистры 0..26) —
опрос счётчика отключается с логом и **legacy-файл `dds238.json` НЕ используется** (иначе
молча опрашивался бы другой счётчик с чужим IP). Fallback на `dds238.json` — только когда
раздела `meter` в конфиге НЕТ вовсе. `register_count == 0` → дефолт 27.

**Две задачи:**
1. **Мгновенные значения** — как у МАП: отдельный **1-сек цикл** `runMeterPoll` (`meter_poller.go`),
   не в 10-сек циклах инверторов. Читает регистры 0..26 (`read_holding_registers(0,27)`), пишет в Redis
...
## ANT BMS (bmslistener → read_bms.php → пулер sunReceiver)

Цепочка данных: **bmslistener** (C-демон на Малине, 192.168.13.60; пассивно слушает
ANT BMS по USB-serial, 140-байтные live-кадры 19200) публикует коллекцию активных
батарей в **System V shm ключ 2018** (`{"updated":<epoch>,"devices":[{...},...]}` + `#EOF`).
**read_bms.php** (новый скрипт на Малине `/settings/html/read_bms.php`, исходник в
`bmslistener/web/`) читает shm 2018 по аналогии с `read_json.php` и отдаёт JSON
(Basic-auth, как весь веб-API Малины; существующие скрипты Малины не модифицируются).
Детали протокола/демона — `docs/antbms.md`, `docs/antbms-worklog.md`.

- **Конфиг**: поле `mppt.bms_path` sunReceiver.json (напр. `/read_bms.php`); хост и
  Basic-auth — те же, что у MPPT (раздел `mppt`). Поле пусто/отсутствует — опрос BMS
  отключён (`loadBmsSite` → nil).
- **Пулер** — `bms_poller.go`: отдельный 1-сек цикл `runBmsPoll` (не в 10-сек циклах
  инверторов), один HTTP-запрос `read_bms.php` за цикл. Состав батарей динамический
  (как MPPT): устройство появляется/исчезает по факту наличия в ответе; исчезнувшие
  удаляются из Redis. При ошибке запроса предыдущее состояние сохраняется.
  **5-минутные усреднённые точки** (`bms_accumulator.go`): 1-секундные снимки
  накапливаются В ПАМЯТИ (BMS, в отличие от инверторов, не имеет ряда в Redis до
  усреднения) и по завершении каждого 5-минутного промежутка пишутся в Redis
  (ряд, окно 2 календарных суток) и в PG (вечно). Усредняются мгновенные параметры
  (ток/мощность/SOC/ёмкости/ячейки/температуры); флаги (MOS/балансировка/cell_count)
  и счётчик `frames` — по последнему снимку промежутка. При остановке пулера
  накопленный (возможно неполный) промежуток дописывается; при перезапуске
  промежуток до момента старта восстановлению не подлежит (источник — только живой
  опрос). **Один (устройство, промежуток) может записываться дважды** (drain
  неполного промежутка при остановке + продолжение того же промежутка новым
  процессом): запись ЗАМЕНЯЕТ старую точку того же устройства на том же score —
  `SaveBMSSeries` (ZREM старых member'ов с тем же score и именем + ZADD) и
  `InsertBMSAveraged` (ON CONFLICT DO UPDATE) — в Redis и PG остаётся ровно одна
  (последняя, более полная) точка на промежуток. Иначе в ZSET накапливались бы два
  member с одинаковым score — «ступенька» на графике (два агрегата в одну
  засечку времени).
- **Redis-ключи**:
  - **`sunreceiver:bms`** — HASH текущего состояния: поле = `deviceName`
    ...
    max/min/avg_cell_v + индексы, frames.
   - **`sunreceiver:bms:series:<YYYY-MM>`** — ZSET 5-минутных усреднённых точек:
     score = Unix (сек, начало промежутка), member = JSON `bmsSeriesPoint`
     (name + ts + `bmsAveraged`). Хранятся последние 2 календарных суток (чистка
     `PurgeOld` — она же чистит этот ряд). `QueryBMSSeries(name, from, to)` — точный
     срез по имени для графиков. На (устройство, score) — ровно одна точка:
     повторная запись заменяет старую (см. выше).
  - BMS **НЕ в `sunreceiver:current`** (нет универсального контракта values).
- **PostgreSQL**: таблица **`sunreceiver.bms_averages(name, ts, values jsonb,
  PK (name, ts))`** — вся история 5-минутных усреднённых точек BMS; `values` —
  `bmsAveraged`. PK (name, ts) даёт эффективную выборку «конкретная BMS за
  диапазон» (узкий индексный range-scan) — `pgStore.BMSAverages(name, start, end)`;
  вставка — `pgStore.InsertBMSAveraged` (upsert: ON CONFLICT DO UPDATE — поздняя,
  более полная запись того же промежутка побеждает, как и в Redis `SaveBMSSeries`).
  Выборка «конкретная BMS за диапазон» для графиков — PK (name, ts): узкий
...
- **`deviceName`** — первое поле объекта устройства, генерируется bmslistener как
  ...
  декодированный `r.URL.Path` с TrimPrefix. **Разовая миграция имён (2026-09-16)**:
  до деплоя bmslistener (батч 5) имя было `AntBms <ёмкость> A/h` без порта — вся
  история (Redis `sunreceiver:bms:series:*` + PG `sunreceiver.bms_averages`) под
  старым именем, страница деталей (имя из `sunreceiver:bms`) под новым — графики
  за старый период были пустыми. История переименована разово (Lua-скрипт в Redis:
  точное совпадение `"name":"<старое>"` + дедупликация (устройство, 5-мин
  промежутка) по большему `samples`; в PG — `UPDATE name` с тем же правилом
  для коллизий по PK (name, ts)). Если формат имени снова изменится — повторить
  аналогичную миграцию в обоих хранилищах.
- **Температуры T1–T6**: производитель не публикует соответствие каналов и мест;
  ...
  до 4 внешних датчиков; параметры защит: «температура батареи» / «силовых
  ключей») подписаны: T1 — Батарея 1, T2 — Батарея 2, T3 — Силовая плата (силовые
  ключи), T4 — Плата управления, T5/T6 — Резерв (не выводятся в приложении ANT).
  Аргументация и источники — `docs/antbms/antbms-protocol-status-frame.md`,
  «Датчики температуры T1–T6». В нашей установке все 6 каналов показывают
  одинаковую температуру (один корпус).
- **Дашборд**: на главной — группа «BMS (ANT батарея)» с кнопками-батарейками
  (заполнение = SOC, цвет по SOC; обновление раз в **минуту** — отдельный
  `tickBMS`, не в 1-сек tick). Клик — страница `/bms/<name>` (шаблон
  `bmsDetailPage`, обновление раз в **секунду**): KPI-плашки (SOC с мини-батарейкой,
  напряжение пакета = сумма ячеек, ток, мощность, ёмкость), **напряжения ячеек —
  батарейки** (заполнение по шкале LiFePO4 3.00–3.65 В; самая высокая ячейка —
  **красная**, самая низкая — **синяя**, остальные — **зелёные**; напряжения и
  разброс — **в вольтах**, напр. `3.241 В`, `разброс: 0.005 В (макс 3.243 / мин
  3.238 В)`), температуры (бары T1–T6 с русскими именами канала + примечание о
    реконструкции), MOS-флаги (Заряд/Разряд/Балансировка ВКЛ/ВЫКЛ), **графики за
    последние сутки** (5-минутные средние: окно 2 суток — из Redis, старше — из PG
    `bms_averages`, сшивка по `recentCutoff`, `/api/bms/<name>/series?from=&to=`,
   Chart.js; **обновление раз в минуту** — destroy + rebuild, инстансы в
   `BMS_CHARTS`, не в `window[id]` (там элемент canvas с этим id)): заряд (SOC, %),
   напряжение пакета (V, сумма ячеек), ток (A, ± ось посередине), мощность (W,
   ± ось посередине), напряжения всех ячеек на одном графике (V), температуры
   T1–T4 (батарея/силовые ключи/плата) на одном графике. API: `/api/bms` (все, для
  батареек), `/api/bms/<name>` (одна, для страницы деталей; 404, если нет в Redis),
  `/api/bms/<name>/series` (ряд 5-минутных точек за период, по умолч. 24 ч).
...
6. ~~Redis-хранилище~~ — готово (`redis_store.go`): HASH `current` + месячные ZSET `series:<YYYY-MM>`, запись вместо JSON-файлов, TTL на сегмент; Redis с persistence (RDB+AOF), при пустом Redis данные восстанавливаются из PG.
...
**Всегда сразу после любых правок кода — commit (по-русски) + push + деплой** (пункты 1–3 выше), без ожидания отдельной команды пользователя.
