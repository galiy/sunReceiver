...
**Интеграция ЦЕЛЬ «КЭС» в `main.go` (`kindMAP`):** `mapMAPRegisters` заполняет контракт из байт-ячеек МАП (см. в main.go). `pollDevice` (case kindMAP) читает блоки 0x400 (режим/АКБ) и 0x520 (`_I_Akb_MPPT`) через `modbusmap` и собирает `values`:
- `l1_voltage` = `_UAcc_med_VH/VL` (0x405/0x406), `(VH*256+VL)/10` (В) — источник — напряжение АКБ МАП, т.к. V_Bat самих контроллеров гейт не отдаёт.
- `l1_current` = сумма токов `_I_Akb_MPPT` (0x530+2i / 0x531+2i, ×16; H с бит 0x80 — «нет данных», слот пропускается).
