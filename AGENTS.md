...
- `main.go` — poller: каждые 10 с параллельно (goroutine) TCP-опрос целей из `config.json` (порт 8899). Список целей читается из **`config.json` рядом с бинарником** (`os.Executable()`; при `go run .` — fallback в CWD) через `loadConfig` в `targets []invTarget` {IP, LoggerSN, Kind}: `type` "deye"→`kindDeyeString`, "sofar"→`kindSofar`. Deye — реальный SN даталоггера (`ReadRegistersDeye`), Sofar — `ReadRegisters` (отвечает и при реальном SN). Маппинг регистров Sofar в `regMap` (`mapSofarRegisters`), Deye string в `deyeRegMap` (`mapDeyeRegisters`, у каждого сенсора два имени: `Name` для raw_registers и `Tag` для контракта в values). JSON: на каждый инвертор отдельный файл `data/<YYYY-MM-DD>/<IP>-<HHMMSS>.json` (дата в директории, время опроса и IP — в имени файла; внутри IP нет). Файл пишется только при успешном чтении данных (`HasData`); при `heartbeat_only`/`no data`/ошибке не сохраняется. Структура: `{timestamp, device_sn, values, raw_registers}`. `values` — **универсальный контракт** (см. ниже); `raw_registers` — **именованные** ключи: Sofar — все 40 регистров 0x0000-0x0027 из `SOFARMap.xml` (проект github.com/galiy/Sofar_LSW3) по именам из `regMap`; Deye — имена из `deyeRegMap` (string-группа kbialek/deye-inverter-mqtt), 32-битные — `<имя>_lo`/`<имя>_hi`; адреса без имени (недокументированные пробелы: 0x3D, 0x41-0x45, 0x51, 0x53, 0x55, 0x59, 0x5C-0x6C) остаются hex. Таблицы имен — `sofarRawRegNames`/`deyeRawRegNames` (`buildRawRegNames`).

### Универсальный контракт `values` (одинаков для Deye и Sofar)
`values` хранит теги с **одинаковыми именами и единицами измерения** для обеих марок; единица зашита в суффикс имени. Общие поля присутствуют у обоих брендов, специфичные — только у своего.
- **Общие (одинаковые теги/единицы):** `pv1/pv2_voltage` (V), `pv1/pv2_current` (A), `pv1/pv2_power` (W), `ac_active_power` (W), `ac_reactive_power` (**var**), `grid_frequency` (Hz), `l1/l2/l3_voltage` (V), `l1/l2/l3_current` (A), `energy_today` (**kWh**), `energy_total` (**kWh**), `time_today` (min, только Sofar — время генерации за день; у Deye `uptime` = минуты с включения, семантика другая).
- **Sofar-only:** `inverter_status` (string), `fault_1..5` ([]string), `country` (string), `dc_*` нет; `temperature_module`/`temperature_inner` (C), `bus_voltage` (V), `time_total` (h), `insulation_*` (Ohm), `pv1_sample_cpu_*`, `countdown_time`, `alert`, `input_mode`, `comm_board_msg`.
- **Deye-only:** `dc_total_power` (W), `ac_apparent_power` (W), `grid_l12/l23/l31_voltage` (V), `temperature_radiator`/`temperature_igbt` (C), `uptime` (min), `load_power` (W), `grid_power` (W), `energy_sold_today/total`, `energy_bought_today/total`, `energy_load_today/total` (kWh), `pv3/pv4_*` (на 2-цепных = 0).
...
- **Данные, которых нет у обеих:** Deye не отдаёт статус/фолты/страну/изоляцию; Sofar не отдаёт apparent/reactive-load/energy-sold-bought. В `values` отсутствующие поля просто отсутствуют.
