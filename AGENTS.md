...
- **Redis-ключ `sunreceiver:bms`** — HASH: поле = `deviceName` (напр. `AntBms 320 A/h`),
  значение = JSON устройства `bmsDevice` (bms_poller.go): deviceName, port,
  timestamp/time (актуальность кадра), cell_count, cells_v[] (V), current_a (A,
  знаковый), soc (%), capacity_ah, remaining_ah, temperatures_c[] (°C),
  charge_mos/discharge_mos/balancer (0/1), power_w (W), max/min/avg_cell_v + индексы,
  frames. **Без временного ряда, без PG, НЕ в `sunreceiver:current`** (у BMS нет
  универсального контракта values) — только актуальное состояние.
...
- **Дашборд**: на главной — группа «BMS (ANT батарея)» с кнопками-батарейками
  ...
  **красная**, самая низкая — **синяя**, остальные — **зелёные**; рядом с заголовком —
  **разброс в мВ** (макс/мин)), температуры (бары T1–T6), MOS-флаги (Заряд/Разряд/
  Балансировка ВКЛ/ВЫКЛ). API: `/api/bms` (все, для батареек), `/api/bms/<name>`
  (одна, для страницы деталей; 404, если нет в Redis). Все надписи на русском.
