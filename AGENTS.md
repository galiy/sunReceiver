...
Список опрашиваемых инверторов задаётся в **`sunReceiver.json` рядом с исполняемым файлом** (`os.Executable()`; при `go run .` — fallback в CWD): `{"targets": [{"ip", "name", "type": "deye"|"sofar"|"map"|"mppt", "logger_sn": uint32, "unit": int, "slot": int}]}`. `name` — логическое имя инвертора (обязательно). `type:"map"` — МАП Титанатор по Modbus TCP (unit — адрес, по умолч. 1; slot — не задан → батарея/сеть для дашборда). `type:"mppt"` — MPPT-контроллер через веб-API ПАК «Малина» (`slot` — индекс контроллера в ответе `read_json.php?device=mppt`). Опрос Deye/Sofar — раз в 10 секунд; МАП и MPPT — 1 раз в секунду (с сохранением 1 точки за 10 с).
...
- В `sunReceiver.json` тип **`mppt`** → `kindMPPT` — источник данных MPPT-контроллеров: **веб-API ПАК «Малина»** `read_json.php?device=mppt` (HTTP Basic-auth; конфиг `base_url`/путь/логин/пароль — в скрытом `malina.json`). Три цели `MPPT-1/2/3` (192.168.13.60, `slot` 0/1/2 = индекс в массиве ответа). Напрямую видны параметры панелей, которых нет в Modbus-гейте: Vc_PV, Ic_PV, P_PV, V_Bat, I_Ch, P_Out + `timestamp` актуальности данных. Реализация — `mppt_api.go` (`mpptSite`, `FetchMPPTs`, `mapMPPTAPI`), коннектор к `read_json.php` описан в `docs/read_json.md`.
- Тип **`map`** → `kindMAP` (Modbus TCP, `modbusmap/` + `mapClientFor`) — оставлен для **данных батареи и сети МАП на дашборд** (grid/battery). Цель `MAP (батарея/сеть)` (192.168.13.74, unit 1, без `slot` = агрегат). Пер-слотовый MPPT через Modbus больше не опрашивается — MPPT переехали на HTTP.

**`values` углов МАП/MPPT (теги добавлены в общий контракт `commonContractTags`):**
- MPPT API (`kindMPPT`, `mapMPPTAPI`): `pv1_voltage/current/power` = Vc_PV/Ic_PV/P_PV панелей контроллера; `l1_voltage` = V_Bat (напряжение АКБ); `l1_current` = I_Ch (ток заряда); `ac_active_power` = P_Out (мощность на выходе контроллера). `timestamp` снимка = поле `timestamp` ответа API.
...
- PollDevice (case `kindMAP`) читает блоки `modbusmap`: 0x400 (`_UAcc_med` 0x405/0x406), 0x420 (`_UNET` 0x422), 0x530 (`_I_Akb_MPPT`), 0x580 (`_PNET_Sign_P` 0x587, `_PNET` 0x59A/0x59B, `_PLoad` 0x59E/0x59F).
