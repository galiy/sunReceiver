...
Список опрашиваемых инверторов задаётся в **`config.json` рядом с исполняемым файлом** (`os.Executable()`; при `go run .` — fallback в CWD): `{"targets": [{"ip", "name", "type": "deye"|"sofar"|"map", "logger_sn": uint32, "unit": int}]}`. `name` — логическое имя инвертора (обязательно). `unit` — Modbus-адрес МАП (только для `type:"map"`, по умолч. 1). Опрос раз в 10 секунд, парсинг и сохранение в JSON.
...
## МАП Титанатор + MPPT («КЭС», тип `"map"`) — Modbus TCP
Гибридный инвертор **МАП Титанатор** (192.168.13.74, port **502**, unit 1) + до 16 параллельных **MPPT контроллеров** Микроарт (`(C)mART`), к которым подключены панели. МАП отдаёт данные по **Modbus TCP** (не Solarman-кадр): MBAP + func 03. Регистры МАП = **байт-ячейки**: значение ячейки лежит в СТАРШЕМ байте слова, младший байт = следующая ячейка. Реализация: `modbusmap/` (`Client.ReadRegisters` возвращает байты ячеек от start..start+2*count-1; `mapClientFor` в main.go — пул переиспользуемых сокетов). **Таймаут чтения гейта = 3 с** (как `timeout=3` в `docs/map/mapread/mapread.py`; отвечает быстро, ~60-80 мс — ожидание >3 с это ошибка). Карта ячеек — в `docs/map/` (`protocol_MAP_cells_2023_06_27.doc`, `Карта Памяти_MPPT*.xlsx`). `probe/map/` — диагностика чтения ячеек МАП (функ 03, unit, диапазон).

**Главное ограничение (проверено живьём 2026-09-07):** PV-данные солнечных панелей через Modbus МАП получить **невозможно**. МАП отдаёт про MPPT только агрегат: `_I_Akb_MPPT[16]`=0x530 (ток заряда каждого MPPT, 16 бит LE: L=0x530+2i, H=0x531+2i, value=(L+H*256)/16, старший бит H=0x80 = «нет данных»), `_Takb_MPPT[16]`=0x560 (температура), `_UAcc_med_VH/VL`=0x405/0x406 (напряжение АКБ: (VH*256+VL)/10). Индивидуальные V_Bat, Vc_PV/Ic_PV/P_PV и энергия контроллеров через МАП-гейт НЕ отдаются (V_Bat через +0x1F0 = 0). PV берутся на следующем этапе из другого источника.

**Интеграция ЦЕЛЬ «КЭС» в `main.go` (`kindMAP`):** каждый MPPT-контроллер — отдельная цель/колонка дашборда. В config.json три цели `MAP MPPT-1/2/3` (192.168.13.74, unit 1) с `slot` 0/1/2; `slot<0` (без поля) = агрегат по всем. `mapMAPRegisters(cells, slot)` заполняет контракт из байт-ячеек МАП (см. в main.go). `pollDevice` (case kindMAP) читает блоки 0x400 (режим/АКБ) и 0x520 (`_I_Akb_MPPT`) через `modbusmap` и собирает `values`:
- `l1_voltage` = `_UAcc_med_VH/VL` (0x405/0x406), `(VH*256+VL)/10` (В) — источник — напряжение АКБ МАП, т.к. V_Bat самих контроллеров гейт не отдаёт. Одинаково для всех MPPT.
- `l1_current` = ток заряда конкретного MPPT `_I_Akb_MPPT[slot]` (0x530+2*slot / 0x531+2*slot, ×16; H с бит 0x80 — «нет данных», ток=0); при slot<0 — сумма по всем активным.
- `ac_active_power` = `l1_voltage × l1_current` (Вт).
- `grid_frequency` = 0 (частоту сети МППТ не отдаёт).
- `pv1/pv2_*`, `ac_reactive_power`, `l2/l3_*` в `values` не пишутся (нет данных).
- **Ключ устройства в Redis/PG = `devKey(t)`**: для МАП с slot это `IP#mppt<slot>` (напр. `192.168.13.74#mppt0`), чтобы 3 контроллера одного гейта не сливались в одну колонку/ряд. Сетевой адрес — по-прежнему `t.IP` (общий сокет `mapClientFor`).
- **Кадр опроса МАП — 1 раз в секунду** (отдельный цикл `runMapPoll`, не в общем 10-сек `doPoll`), в Redis пишется через `SaveSnapshotWindow` (`redis_store.go`): score точки = начало 10-секундного окна (`ts.Truncate(10s)`), в пределах одного окна каждая новая запись **заменяет** предыдущую (ZREM+ZADD), поэтому в Redis по каждому МАП-контроллеру остаётся **ровно одна строка за каждые 10 секунд** (остальные — актуальное последнее значение окна; HASH current обновляется каждый опрос). Соответственно аккумуляцией PG это усредняется как у остальных (~1 точка за 10 с с дискретностью 5 мин).
