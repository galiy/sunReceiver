...
- **Страница электроэнергии `/energy`** (кнопка «Электроэнергия» на главной) — посуточные и помесячные тарифы DDS238 из `daily_tariffs` (после переноса со страницы графиков): график «по дням» (по умолч. текущий месяц) + график «по месяцам» (по умолч. текущий год). У каждого графика **независимый** диапазон (пресеты + свой «С/по»), между собой не синхронизируются. Данные — `/api/tariffs?from=&to=` (по умолч. текущий месяц), `pgStore.DailyTariffsRange` (финализированные дни в диапазоне). Зум по X как на других графиках: Ctrl+колесо / drag-панорама + кнопка «Сброс зума» (десктоп), щипок/свайп/двойной тап (mobile, `srTouchChart`; category-шкала — min/max по индексам столбцов, `minRange:3`); плагин chartjs-plugin-zoom загружается на странице. **Важно**: drag-панорама плагином (Hammer-рекогнизитор Pan) требует загрузки **hammer.js** — скрипт `hammerjs@2.0.8` грузится ПЕРЕД chartjs-plugin-zoom на всех трёх страницах с графиками (/charts, /energy, /bms); без него `window.Hammer` отсутствует и плагин молча не инициализирует жесты (работает только Ctrl+колесо — нативный wheel-слушатель). На «Электроэнергия» (category-шкала) плагинный `pan` выключен (`pan.enabled:false`) — его category-панорама перемещается только если дельта ОДНОГО события превышает ширину столбца; вместо неё — кастомный `bindCategoryPan` (десктоп): mousedown запоминает окно, mousemove сдвигает его на целое число столбцов по суммарному пиксельному сдвигу через `zoomScale` («Сброс зума»/лимиты продолжают работать); на touch панораму делает `srTouchChart`.
- **Догрузка данных при сдвиге графика (все страницы с графиками)**: при панораме (drag/swipe) или зуме-ауте, если видимое окно уходит за пределы загруженных точек, диапазон расширяется и данные перечитываются из API, текущее окно сохраняется. `/charts` и `/bms/<name>` (time-шкалы): колбэки chartjs-plugin-zoom → `maybeExtendAndLoad`/`bmsMaybeExtend` (буфер = max(25% окна, 30 мин) — микро-сдвиги догрузку не запускают; в будущее не догружается; `preserveZoom` сохраняет окно при пересоздании). **Триггер — только в КОНЦЕ жеста** (`pan.onPanComplete`/`zoom.onZoomComplete`): ранний вариант с `pan.onPan` (дебаунс 350 мс) удалён — догрузка посреди драга пересоздавала чарт под курсором и ломала Hammer-жест (график «прыгал» на позицию до завершения сдвига). **Фолбэк `pointerup` (debounce 80 мс)** на /charts и /bms: если hammer panend не сработал (напр. неполный жест), любое отпускание указателя запускает проверку догрузки после жеста (в пределах данных — no-op). **Ловушка**: плагин читает колбэки панорамы из подобъекта `pan:`, а `onZoomComplete` — из корня `zoom:`; положенные на «не тот» уровень колбэки молча не срабатывают (проверено по исходникам v2.0.1). Mobile — `onPanEnd` в `srTouchChart` (вызывается по завершении свайпа/щипка). **Pre-injection окна**: при сохранении зума (preserveZoom) min/max вшиваются в opts ДО `new Chart()` (`renderChart`/`bmsRender`) — иначе первый draw на полном диапазоне через afterDraw-синхронизацию («checkZoomSync») транзитно «раскачивает» остальные графики страницы, и после догрузки график «прыгал обратно» на полный период. `/energy` (category-шкала): окно за данные не уходит (`zoomScale` зажимает [0..N-1]), поэтому догрузка — по **направлению** упершегося края: `bindCategoryPan` (десктоп) и `srTouchChart` (mobile) запоминают `edge:'left'/'right'`, `maybeLoadMore(direction)` расширяет [selFrom, selTo] на ~50% видимого окна (влево лимит ~2 года/24 месяца, вправо — до текущего дня/месяца), окно сохраняется через `pendingPreserve` с **якорной меткой** (первая метка до догрузки; при вставке новых дней/месяцев слева все индексы сдвигаются на её новый индекс).
