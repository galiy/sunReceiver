...
- **Зум и панорама (все страницы с графиками)** — только горизонтальные: `mode:'x'` в опциях chartjs-plugin-zoom у всех графиков (/charts, /energy, /bms/<name>), mobile pinch/pan через `srTouchChart` — тоже только по X. Старый вертикальный зум графика напряжений на /charts (`zoomMode:'xy'`, `limits.y/y1`) удалён 2026-09-16 — теперь все графики зумятся только по горизонтали, как и остальные. **Авто-догрузки данных при сдвиге/зуме НЕТ** (фича удалена 2026-09-16, на проде оказалась ненадёжной): `maybeExtendAndLoad`/`bmsMaybeExtend`/`maybeLoadMore`, колбэки `pan.onPanComplete`/`zoom.onZoomComplete`, фолбэк `pointerup` и отслеживание `edge:'left'/'right'` в `bindCategoryPan`/`srTouchChart` удалены — если окно ушло за пределы загруженных точек, за границей данных просто пусто (граница данных = выбранный период). Окно зума/панорамы сохраняется при ПЕРИОДИЧЕСКОЙ перестройке графиков (раз в минуту): `preserveZoom` + min/max вшиваются в opts ДО `new Chart()` (`renderChart`/`bmsRender`) — иначе первый draw на полном диапазоне через afterDraw-синхронизацию (checkZoomSync на /charts, `bmsZoomSyncPlugin` на /bms) транзиторно «раскачивает» остальные графики страницы, и после обновления график «прыгает обратно» на полный период.
