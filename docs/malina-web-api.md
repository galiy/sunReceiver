# Web-интерфейс ПАК «Малина» (192.168.0.60) — PHP-обёртки и API
...
- ХОСТ: Raspberry Pi, Raspbian 8 (jessie), hostname `malina.localhost`.
  ...
  `/settings` (mmcblk0p3). Веб-интерфейс — PHP 5, сервер — Microsoft-IIS/6.0
   на памяти (вероятно nginx-обвязка), Basic-auth (`admin`/пароль из раздела `map`
   в `sunReceiver.json`).
...
- Конфиг доступа (Basic-auth) — раздел `map` файла `sunReceiver.json` (не в git).
...
Доступ к веб-API (Basic-auth) задаётся в **разделе `map` файла `sunReceiver.json`**
(см. `mppt_api.go`, `loadMPPTSite`). Раньше был отдельный файл `malina.json` — он
удалён, все поля переехали в `sunReceiver.json`.

| Поле | Назначение |
|---|---|
| `mppt.base_url` | адрес ПАК «Малина», например `http://192.168.0.60` |
