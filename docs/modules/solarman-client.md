...
- `NewClient(addr string, opts ...Option)` — TCP-клиент с зависимым от логгера
  поведением чтения (логгеры шлют данные МЕДЛЕННО, частями).
...
Опции (`Option`): `WithTimeout`, `WithIdleWindow` (пауза «тишины» перед завершением
чтения; для Sofar больше — паузы между кусками до ~6.5 с), серийный номер
(`WithSerial`) и др. Цикл чтения — до «тишины» `IdleWindow`, первый байт — до
`Timeout`.
