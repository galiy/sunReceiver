...
- **MAP API** (`mapMAPAPI`, при `map.rs485.disabled=true`): тот же контракт из
  ...
  `_PNET_calc`). Знак зависит от `_Inet_flag` ПО «Малины»: при `_Inet_flag=1` API отдаёт
  `_PNET_calc` с обратным знаком (при потреблении `<0`) — инвертируем; при `0`/отсутствии
  значение уже контрактное. Итог: потребление из сети «+», отдача в сеть «−». Правка только
