...
> ## Статус: ОТКЛЮЧЕНО И УДАЛЕНО (2026-09-24)
>
> На `.253` установлен штатный физический USB Bluetooth-адаптер (Realtek
> `0bda:8771`, `RTK_BT_5.0`), поэтому виртуальный vhci-контроллер из MediaTek
> больше не используется. Юниты, скрипты и `modules-load.d` удалены с обоих
> хостов, проброс отключён.
>
> Ниже — **историческое описание схемы**. Снимки реальных файлов с хостов — в
> [`backup/`](backup/), там же порядок восстановления — [`backup/NOTES.md`](backup/NOTES.md).
...
используется CE308-пулером (см. [`../modules/ce308.md`](../modules/ce308.md)).
