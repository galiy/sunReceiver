...
- `-u` — Modbus-адрес МАП для опроса при автопоиске (по умолчанию `1`, допустимо `1..247`).
- `--sn N` — ожидаемый **серийный номер** МАП (целое `0..65535`; десятичное или
  `0x...`-HEX; можно указать заглушку `CHANGE_ME` — тогда фильтр по серийнику
  просто **отключается**, а не превращается в `sn=0`). Если задан числом, кандидат
  принимается только при точном совпадении `_SerialNum0/1` (0x18 мл., 0x19 ст.).
- `--sn-letter C` — ожидаемая буква серийного номера (`_SerialNum3=0x23`, ASCII).
- Автопоиск (по умолчанию, без `-d`): перебираем свободные `/dev/ttyUSB*`,
  `/dev/ttyACM*` и `/dev/serial/by-id/*` (дубли одного порта дедуплицируются;
  занятые другими процессами пропускаем) и коротким Modbus-опросом блока
  идентификации (`0x03`, ячейки `0x00..0x2F`) ищем свой МАП. Критерий: валидный
  кадр + сигнатура семейства МАП (`_VerPO(0x02)!=0`, `_DevOpt(0x07) in {1,2,3}`)
  + (при `--sn`) совпадение серийника. За один проход проверяется **не более
  одного** кандидата, чтобы не блокировать event loop на всё время сканирования;
  непроверенные кандидаты обрабатываются при последующих запросах. «Залипаем» только
  на найденном; при пропаже/10 подряд неудачных транзакциях — ищем снова. Найденные
  `sn/devopt/verPO` пишутся в лог.
...
- Устройство ищется автоматически среди **свободных** последовательных портов
  `/dev/ttyUSB*`, `/dev/ttyACM*` и `/dev/serial/by-id/*` коротким Modbus-опросом
  (номера `/dev/ttyUSB*` плавают после ребута) — по аналогии с `bmslistener`; занятые
  порты (напр. BMS-адаптеры, которые держит `bmslistener`) пропускаются.
...
- **Клиентские TCP-сокеты неблокирующие** (`O_NONBLOCK`) с очередью ответа на клиента:
  частичная запись дописывается по `POLLOUT`, медленный/нечитающий клиент не в состоянии
  подвесить шлюз, а сброс (`EPIPE`/`ECONNRESET`) трактуется как дисконнект. Для соединений
  включены `SO_KEEPALIVE` (60/10/3 c) и таймаут бездействия 5 минут: 16 слотов не выедаются
  «полуоткрытыми»/молчащими клиентами.
- Автопоиск за один проход опрашивает только одного кандидата (event loop не блокируется
  на `PROBE_MS * N`), остальные проверяются при последующих запросах.
