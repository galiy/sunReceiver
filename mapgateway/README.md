...
- `--sn N` — ожидаемый **серийный номер** МАП (целое `0..65535`; можно указать
  заглушку `CHANGE_ME` — тогда фильтр по серийнику просто **отключается**, а не
  превращается в `sn=0`). Если задан числом, кандидат принимается только при
  точном совпадении `_SerialNum0/1` (0x18 мл., 0x19 ст.).
