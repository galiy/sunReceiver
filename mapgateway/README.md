...
- Автопоиск (по умолчанию, без `-d`): перебираем свободные `/dev/ttyUSB*` (занятые другими
  процессами пропускаем) и коротким Modbus-опросом (чтение регистра `0x0400`) ищем свой МАП;
  «залипаем» только на найденном. При пропаже/10 подряд неудачных транзакциях — ищем снова.
